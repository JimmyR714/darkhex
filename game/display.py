...
class TrainingFrame(tk.Frame):
    ...
    def __init__(self, master: DisplayWindow):
        super().__init__(master=master, relief=tk.RAISED, bd=2)
        #add title
        tk.Label(master=self, text="Training Frame").grid(row=0, column=0, sticky="n")
        self.iters = tk.StringVar()
        tk.Label(master=self, text="Iterations").grid(row=1, column=0, sticky="n")
        tk.Entry(self, textvariable=self.iters).grid(row=2, column=0, sticky="n")
        sim_options = [
            "Offline",
            "Online vs Offline",
            "Online"
        ...
        self.current_agents = []
        self.frm_current_agents = tk.Frame(self, relief=tk.RAISED, bd=2, height=300)
